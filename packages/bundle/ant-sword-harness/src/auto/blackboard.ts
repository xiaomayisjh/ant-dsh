...
export class BlackboardService extends Service {
  ...
  private cycleOf(sessionId: string): number {
  ...
  async setStatus(session: Session, nodeId: string, status: IntentStatus): Promise<void> {
    const domain = await this.domainReady
...
export function applyBoardProjection(
  ...
  if (data.op === 'status') {
    return { ...current, nodes: current.nodes.map(n => n.id === data.nodeId ? { ...n, status: data.status as IntentStatus } : n) }
  }
